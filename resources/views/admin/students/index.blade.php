...
@section('container')
    <div class="w-11/12 mt-6 ml-10">
        <div class="mb-2 flex justify-between">
            ...
            <div>
            <a href="/mahasiswa/create" class=" text-white block rounded-2xl py-2 px-4 border border-amber-300 bg-blue-400"> <i class="fas fa-plus"></i>  data</a>
        </div>
        </div>
            ...
            <table class="min-w-full bg-white">
                <thead class="bg-[#374151] text-white text-base">
                    <tr class="border-b border-gray-200">
                        ...
                        <th class="w-1/3 px-4 py-2 text-left  font-medium  uppercase tracking-wider">Nama </th>
                        <th class="w-1/3 px-4 py-2 text-left  font-medium  uppercase tracking-wider">Kelas </th>
                ...
                <tbody class="text-gray-800 text-sm">
                    @foreach ($students as $data)
                        <tr>
                            ...
                            <td class=" text-left py-3 px-4">{{ $data->kelas->name }}</td>
                            ...
                            <td class=" text-left py-3 px-4 flex">
                                ...
                                <form action="{{ route('students.destroy',$data->id) }}"  method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus user ini?');">
                                    @csrf
                                    @method('DELETE')
