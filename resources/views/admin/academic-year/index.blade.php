                    ...
                    @foreach ($academicYear as $datas)
                        <tr>
                            ...
                            <td class="w-1/3 text-left py-3 px-4">
                                @if ($datas->is_active)
                                    <span
                                        class="text-white font-semibold inline-block w-14 text-center rounded-3xl bg-green-400">
                                        Aktif</span>
                                @else
                            ...
                            <td class="w-1/3 text-left py-3 px-4 flex">
                                <button
                                    ...
                                    data-id="{{ $datas->id }}" data-name="{{ $datas->name }}"><i
                                        class="fas fa-edit"></i> </button>
                                <form action="{{ route('academicyears.destroy', $datas->id) }}" method="POST"
