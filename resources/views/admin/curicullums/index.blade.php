            ...
            <table class="min-w-full bg-white">
                <thead class="bg-[#374151] text-white text-base">
                    <tr class="border-b border-gray-200">
                        ...
                        <th class="w-1/3 px-4 py-2 text-left  font-medium  uppercase tracking-wider">Kurikulum</th>
                        <th class="w-1/3 px-4 py-2 text-left  font-medium  uppercase tracking-wider">Program Studi </th>
                        <th class="w-1/3 px-4 py-2 text-left  font-medium  uppercase tracking-wider">Tahun Mulai </th>
                        <th class="w-1/3 px-4 py-2 text-left  font-medium  uppercase tracking-wider">Aktif </th>
                        <th class="w-1/3 text-center px-4 py-2 uppercase font-semibold ">Action </th>
                    </tr>
                </thead>
                <tbody class="text-gray-800 text-sm">
                    @foreach ($curicullums as $data)
                        <tr>
                            <td class=" text-left py-3 px-4">{{ $loop->iteration }}</td>
                            <td class=" text-left py-3 px-4">{{ $data->name }}</td>
                            <td class=" text-left py-3 px-4">{{ $data->studyProgram->name }}</td>
                            <td class=" text-left py-3 px-4">{{ $data->start_year }}</td>
                            <td class=" text-left py-3 px-4">
                               @if ($data->is_active)
                                    <span class="w-14 inline-block rounded-3xl text-white bg-green-300 text-center font-semibold">Aktif</span>
                                   @else
